...
long Tools::toCompound(long n, long m) { return n * (n + 1) - m - 1; }

long Tools::toFirstIt(long p) { return (long)std::sqrt(p + 1.0); }
...
void Tools::makeUnitMatrix(long size_, std::complex<double> **I_) {
  ...
  for (i = 0; i < size_; i++)
    for (j = 0; j < size_; j++) {
      I_[i][j] = (i == j) ? std::complex<double>(1.0, 0.0)
                          : std::complex<double>(0.0, 0.0);
    }
}

void Tools::pushToMatrix(std::complex<double> **T_small_, long rows_,
