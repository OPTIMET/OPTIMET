...
//------------------------------------------------------------------------------------
//------------------------------------------------------------------------------------
void PeriodicCoupling::compute_AlBe_nmlk(Spherical<double> R,
  ...
  //  int p_max = n_max*n_max + 2*n_max;      // progression relationship - for
  //  n, m
  ...
  //  int p_max1 = (n_max+e)*(n_max+e)+2*(n_max+e); // progression relationship
  //  - for n, m
  ...
  // ----------------------------------
  for (i = 1, n = 1; i < n_Matsize1 - e;
       i++, n++) { // start at n==1 up to and including n_max
    for (j = e, m = -n_max; j < m_Matsize1 - e;
         j++, m++) { // increment by the padded e value
          ...
          for (ii = 0, l = 0; ii < n_Matsize1 - 1;
            ...
            for (jj = 0, k = -(n_max + e - 0); jj < m_Matsize1 - 0;
              ...
              if (abs(k) <= l) {
                ...
                // wavek
                AlBe_nmlkconj[i][j][ii][jj] =
                    -A2 * AlBe2conj + A3 * AlBe3conj + A4 * AlBe4conj;
            ...
            for (jj = 0, k = -(n_max + e - 0); jj < m_Matsize1 - 0;
              ...
              if (abs(k) <= l) {
                ...
                if (BHreg == 0) { // regular    - Bessel
                  ...
                  AlBe_nmlk[i][j][ii][jj] =
                      d_temp * conj(AlBe_nmlkconj[i][m_mirror][ii][k_mirror]);
                } else if (BHreg == 1) { // iregular   - Hankel
                  // obtain mirror coefficient -------------------------------
                  d_temp = pow(-1., d_n + d_l + d_k + d_m); // eqn (C4-B)
                  AlBe_nmlk[i][j][ii][jj] =
                      1. * d_temp *
                      conj(AlBe_nmlkconj[i][m_mirror][ii][k_mirror]);
                }
              }
            } // jj
          }   // ii
...
std::complex<double> PeriodicCoupling::compute_Bnm(int n, int n1, int n2, int m,
                                                   int m1, int m2) {
...
// ---------------------------------------------------------------------------------------
// compute Znmlk(-Rn) function - this is obtained from the scalar
// addition-translation theorem
...
// Rmax is a truncation limit of on the reciprocal lattice - user defined for
// now
int PeriodicCoupling::compute_Znmlk(double a1, double a2,
                                    std::complex<double> waveK, int Rmax,
                                    int n_max,
                                    std::complex<double> ****Z_nmlk) {
  ...
  Spherical<double> Rs;
  Cartesian<double> Rn;
  Cartesian<double> waveKc; // conversion of vKinc into Cartesian coords wrt to
                            // the 2D lattice - make use of components that are
                            // parallel to the 2D lattice only
  ...
  double lam = 1.;
  vKinc = Spherical<double>(2 * consPi / lam, (90 / 180.) * consPi,
                            (90 / 180.) * consPi);
  // end:   AJ - this is temporary - AJ //
  waveKc =
      Tools::toCartesian(vKinc); // (waveKp_x, waveKp_y, waveKp_z) == (K||, Kz)
  ...
  Gp_nmlk = Tools::Get_4D_c_double(n_Matsize1, m_Matsize1, n_Matsize1,
                                   m_Matsize1); // positive Rn
  Gn_nmlk = Tools::Get_4D_c_double(n_Matsize1, m_Matsize1, n_Matsize1,
                                   m_Matsize1); // negative Rn
  for (i = 0; i < n_Matsize1; i++) {
    for (j = 0; j < m_Matsize1; j++) {
      for (ii = 0; ii < n_Matsize1; ii++) {
        for (jj = 0; jj < m_Matsize1; jj++) {
  ...
  // By definition: loop over all lattice constants - excluding the self-term
  // Rn=0  MODINOS 1987 eq (6b)
  for (ll = 1; ll <= Rmax; ll++) {
...
////
///---------------------------------------------------------------------------------------
// compute Omega_nmlk function - this is identically the scalar
// addition-translation theorem
int PeriodicCoupling::compute_OMEGAnmlk(std::complex<double> waveK, int n_max,
                                        std::complex<double> **dataOMEGA_1pq,
                                        std::complex<double> **dataOMEGA_2pq) {

  // first of all increment n_max by 1 to allow for all required values of
  // Z_nmlk
  n_max += 1;
  ...
  int Rmax = 100; // Rmax is a truncation limit of on the reciprocal lattice -
  ...
  for (i = 1, n = 1; i < n_Matsize1 - e;
    ...
    for (j = e, m = -n_max; j < m_Matsize1 - e;
         j++, m++) { // increment by the padded e value
      if (abs(m) <= n) {
        for (ii = 1, l = 1; ii < n_Matsize1 - 1;
             ii++, l++) { // start at n==1 up to and including n_max+e
          for (jj = e, k = -n_max; jj < m_Matsize1 - 0;
               jj++, k++) { // increment by the padded e value
            if (abs(k) <= l) {
              ...
              if (Tools::equalDoubles(alpha_nm_I, 0.) ||
                  Tools::equalDoubles(beta_nm_I, 0.))
                c_temp_I = std::complex<double>(0., 0.);
              else
                c_temp_I =
                    2. * alpha_nm_I * beta_nm_I * Z_nmlk[i][j - 1][ii][jj - 1];
              // II- evaluate all other quantities
              alpha_nm_II = compute_alpha_nm(n, m);
              beta_nm_II = compute_alpha_nm(l, k + 1);
              // check if any of the above are ZERO by default!
              if (Tools::equalDoubles(alpha_nm_II, 0.) ||
                  Tools::equalDoubles(beta_nm_II, 0.))
                c_temp_II = std::complex<double>(0., 0.);
              else
                c_temp_II = 2. * alpha_nm_II * beta_nm_II *
                            Z_nmlk[i][j + 1][ii][jj + 1];
              // III- evaluate all other quantities
              c_temp_III = double(m) * double(k) * Z_nmlk[i][j][ii][jj];
              // obtain OMEGA_1_nmlk --------------------------------------
              OMEGA_1_nmlk[i][j][ii][jj] =
                  1. / std::sqrt(double(n * (n + 1) * l * (l + 1))) *
              ...
              // check if any of the above are ZERO by default!
              if (Tools::equalDoubles(alpha_nm_I, 0.) ||
                  (Tools::equalDoubles(B_nm_I.real(), 0.) &&
                   (Tools::equalDoubles(B_nm_I.imag(), 0.))))
                c_temp_I = std::complex<double>(0., 0.);
              else
                c_temp_I = std::sqrt(8. * consPi / 3.) * pow(-1., k) *
                           alpha_nm_I * B_nm_I *
                           Z_nmlk[i][j + 1][ii - 1][jj + 1];
              // II- evaluate all other quantities
              beta_nm_II = compute_alpha_nm(n, m);
              B_nm_I = compute_Bnm(l - 1, k - 1, 1, 1, l, k);
              // check if any of the above are ZERO by default!
              if (Tools::equalDoubles(beta_nm_II, 0.) ||
                  (Tools::equalDoubles(B_nm_II.real(), 0.) &&
                   (Tools::equalDoubles(B_nm_II.imag(), 0.))))
                c_temp_II = std::complex<double>(0., 0.);
              else
                c_temp_I = -std::sqrt(8. * consPi / 3.) * pow(-1., k) *
                           beta_nm_I * B_nm_I *
              ...
              // III- evaluate all other quantities
              c_temp_III =
                  double(m) * Z_nmlk[i][j][ii - 1][jj] *
                  std::sqrt((l + k) * (l - k) / (2. * l - 1) / (2. * l + 1));
              // obtain OMEGA_1_nmlk --------------------------------------
              OMEGA_2_nmlk[i][j][ii][jj] =
                  1. / std::sqrt(double(n * (n + 1) * l * (l + 1))) *
                  (2. * l + 1) / waveK * (c_temp_I + c_temp_II + c_temp_III);
              // ----------------------------------------------------------
...
int PeriodicCoupling::compute_Ap_vkg(
    int n_max, Cartesian<double> vecKg, std::complex<double> waveK, double Ao,
    Cartesian<std::complex<double>> *dataAp_vkg) {
  ...
  std::complex<double> c_temp(0., 0.), c_temp_I(0., 0.), c_temp_II(0., 0.),
      c_temp_III(0., 0.);
