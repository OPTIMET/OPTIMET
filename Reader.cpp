...
int Reader::readGeometry() {
  ...
  if (geo_node.child("background")) {
    if (std::strcmp(geo_node.child("background").attribute("type").value(),
                    "relative")) {
      std::complex<double> aux_epsilon(1.0, 0.);
      std::complex<double> aux_mu(1.0, 0.);
      aux_epsilon.real(geo_node.child("background")
                           .child("epsilon")
                           .attribute("value.real")
                           .as_double());
      aux_epsilon.imag(geo_node.child("background")
                           .child("epsilon")
                           .attribute("value.imag")
                           .as_double());
      aux_mu.real(geo_node.child("background")
                      .child("mu")
                      .attribute("value.real")
                      .as_double());
      aux_mu.imag(geo_node.child("background")
                      .child("mu")
                      .attribute("value.imag")
                      .as_double());
...
int Reader::readExcitation() {
  ...
  vKinc = Spherical<double>(
      2 * consPi / wavelength,
      ext_node.child("propagation").attribute("theta").as_double() * consPi /
          180.0,
      ext_node.child("propagation").attribute("phi").as_double() * consPi /
          180.0);
  ...
  Eaux = SphericalP<std::complex<double>>(
      std::complex<double>(0.0, 0.0),
      std::complex<double>(
          ext_node.child("polarization").attribute("Etheta.real").as_double(),
          ext_node.child("polarization").attribute("Etheta.imag").as_double()),
      std::complex<double>(
          ext_node.child("polarization").attribute("Ephi.real").as_double(),
...
int Reader::readStructure(xml_node geo_node_) {
  ...
  if (!std::strcmp(struct_node.attribute("type").value(), "spiral")) {
    ...
    if (!std::strcmp(
            struct_node.child("properties").attribute("length").value(), "")) {
      // Distance based simulation;
      d = 2.0 *
          struct_node.child("properties").attribute("distance").as_double() *
          consFrnmTom;
      R = d /
          (4 *
           sin(Theta /
               2.0)); // Remember that the radius needs to be half the length
    } else {
      // Length based simulation
      R = struct_node.child("properties").attribute("length").as_double() *
          consFrnmTom;
    ...
    if (struct_node.child("object").child("properties").attribute("radius")) {
      auto const radius = struct_node.child("object")
                              .child("properties")
                              .attribute("radius")
                              .as_double();
    ...
    std::vector<double> X(
        No - 1); // to store x-axis location of particles for all arm
    std::vector<double> Y(
        No - 1); // to store y-axis location of particles for all arm
    ...
    std::vector<double> x(
        Np - 1); // to store x-axis location of particles for each arm
    std::vector<double> y(
        Np - 1); // to store y-axis location of particles for each arm
    j = 0;
    for (i = 0; i < Np - 1; i++) {
      double x_;
      double y_;
    ...
    for (int arm_i = 0; arm_i < arms; arm_i++) {
      ...
      // particles on arms
      for (i = 0; i < Np - 1; i++) {
        X[j] = x[i] * cos(double(arm_i) * Theta_rot) -
               y[i] * sin(double(arm_i) * Theta_rot);
        Y[j] = x[i] * sin(double(arm_i) * Theta_rot) +
               y[i] * cos(double(arm_i) * Theta_rot);
        j++;
      }
    }
...
int Reader::readOutput() {
  ...
  if (!std::strcmp(out_node.attribute("type").value(), "field")) {
    ...
    run->params[0] =
        out_node.child("grid").child("x").attribute("min").as_double() * 1e-9;
    run->params[1] =
        out_node.child("grid").child("x").attribute("max").as_double() * 1e-9;
    run->params[2] =
        out_node.child("grid").child("x").attribute("steps").as_double();
    run->params[3] =
        out_node.child("grid").child("y").attribute("min").as_double() * 1e-9;
    run->params[4] =
        out_node.child("grid").child("y").attribute("max").as_double() * 1e-9;
    run->params[5] =
        out_node.child("grid").child("y").attribute("steps").as_double();
    run->params[6] =
        out_node.child("grid").child("z").attribute("min").as_double() * 1e-9;
    run->params[7] =
        out_node.child("grid").child("z").attribute("max").as_double() * 1e-9;
    run->params[8] =
        out_node.child("grid").child("z").attribute("steps").as_double();

    if (!std::strcmp(
            out_node.child("projection").attribute("spherical").value(),
            "true")) {
      run->projection = 1;
    } else {
      run->projection = 0;
    ...
    if (!std::strcmp(out_node.child("singlemode").attribute("dominant").value(),
                     "auto")) {
      run->dominantAuto = true;
    } else {
      run->dominantAuto = false;
      run->singleModeIndex.init(
          out_node.child("singlemode").attribute("n").as_int(),
          out_node.child("singlemode").attribute("m").as_int());
      if (!std::strcmp(
              out_node.child("singlemode").attribute("component").value(),
              "TE")) {
  ...
  if (!std::strcmp(out_node.attribute("type").value(), "response")) {
    if (out_node.child("scan").child("wavelength")) {
      ...
      lam_start = out_node.child("scan")
                      .child("wavelength")
                      .attribute("initial")
                      .as_double();
      lam_final = out_node.child("scan")
                      .child("wavelength")
                      .attribute("final")
                      .as_double();
      run->params[0] = out_node.child("scan")
                           .child("wavelength")
                           .attribute("initial")
                           .as_double() *
                       1e-9;
      run->params[1] = out_node.child("scan")
                           .child("wavelength")
                           .attribute("final")
                           .as_double() *
                       1e-9;
    ...
    if (out_node.child("scan").child("radius")) {
      run->params[3] = out_node.child("scan")
                           .child("radius")
                           .attribute("initial")
                           .as_double() *
                       1e-9;
      run->params[4] = out_node.child("scan")
                           .child("radius")
                           .attribute("final")
                           .as_double() *
                       1e-9;
      run->params[5] =
          out_node.child("scan").child("radius").attribute("steps").as_double();
      run->outputType = 12;
    }

    if (out_node.child("scan").child("wavelength") &&
        out_node.child("scan").child("radius"))
      run->outputType = 112;
  }
...
int Reader::readSimulation(std::string const &fileName_) {
  ...
  if (!fileResult) {
    std::cerr << "Error reading or parsing input file " << fileName_ << "!"
              << std::endl;
    return 1;
  }
...
Scatterer Reader::readSphericalScatterer(pugi::xml_node const &node) {
  ...
  if (node.child("cartesian")) // Cartesian coordinates
    ...
    result.vR = {node.child("spherical").attribute("rrr").as_double() *
                     consFrnmTom,
  ...
  if (node.child("properties").attribute("radius"))
    result.radius =
        node.child("properties").attribute("radius").as_double() * consFrnmTom;
  ...
  if (node.child("epsilon") || node.child("mu")) {
    ...
    if (node.child("epsilon").attribute("type").value() ==
        std::string("relative")) {
      // Static values
    ...
    } else if (node.child("epsilon").attribute("type").value() ==
               std::string("DrudeModel")) {
      // Drude model
      auto const plasma_freq = node.child("epsilon")
                                   .child("parameters")
                                   .attribute("plasma_frequency")
                                   .as_double();
      std::complex<double> const damping_freq(
          0, node.child("epsilon")
                 .child("parameters")
                 .attribute("damping_frequency")
                 .as_double());
      result.elmag.init_r(0, aux_mu);
      result.elmag.initDrudeModel_r(plasma_freq, damping_freq, aux_mu);
    } else if (node.child("epsilon").attribute("type").value() ==
               std::string("sellmeier")) {
      // Sellmeier model
      double B1(0.), C1(0.), B2(0.), C2(0.), B3(0.), C3(0.), B4(0.), C4(0.),
          B5(0.), C5(0.);
      B1 =
          node.child("epsilon").child("parameters").attribute("B1").as_double();
      C1 =
          node.child("epsilon").child("parameters").attribute("C1").as_double();
      B2 =
          node.child("epsilon").child("parameters").attribute("B2").as_double();
      C2 =
          node.child("epsilon").child("parameters").attribute("C2").as_double();
      B3 =
          node.child("epsilon").child("parameters").attribute("B3").as_double();
      C3 =
          node.child("epsilon").child("parameters").attribute("C3").as_double();
      B4 =
          node.child("epsilon").child("parameters").attribute("B4").as_double();
      C4 =
          node.child("epsilon").child("parameters").attribute("C4").as_double();
      B5 =
          node.child("epsilon").child("parameters").attribute("B5").as_double();
      C5 =
          node.child("epsilon").child("parameters").attribute("C5").as_double();
      result.elmag.initSellmeier_r(B1, C1, B2, C2, B3, C3, B4, C4, B5, C5,
                                   aux_mu.real());
    } else
