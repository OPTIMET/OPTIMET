...
int Simulation::run()
{
  ...
  Solver solver(&(run.geometry), &(run.excitation), O3DSolverIndirect, run.nMax);
  solver.populate();

  switch(run.outputType)
  {
    case 0: field_simulation(run, solver); break;
    case 11: scan_wavelengths(run, solver); break;
    case 12: radius_scan(run, solver); break;
