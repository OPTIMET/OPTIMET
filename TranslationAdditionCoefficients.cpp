...
constexpr bool is_valid(t_int n, t_int m) {
  return n >= 0 and std::abs(m) <= n;
}
...
t_complex Ynm(Spherical<t_real> const &R, t_int n, t_int m) {
  if (not is_valid(n, m))
    return 0;
  auto const gamma = static_cast<t_real>(2 * n + 1) /
                     (constant::pi * static_cast<t_real>(4)) *
...
t_complex CachedRecurrence::initial(t_int l, t_int k) {
  ...
  auto const bessel =
      regular ? optimet::bessel<Bessel> : optimet::bessel<Hankel1>;
  auto const hb = std::get<0>(bessel(wave, l + 1));
  if (l == 0 and k == 0)
    return hb[l];
  auto const factor =
      std::sqrt(4e0 * constant::pi) * ((l + k) % 2 == 0 ? 1 : -1);
...
t_complex CachedRecurrence::offdiagonal_recurrence(t_int n, t_int m, t_int l,
                                                   t_int k) {
...
t_complex TranslationAdditionCoefficients::operator()(t_int n, t_int m, t_int l,
                                                      t_int k) {
  if (m >= 0)
    return positive(n, m, l, k);

  auto const sign =
      negative.is_regular() ? ((k + l) % 2 == 0) : ((n + l + k + m) % 2 != 0);
