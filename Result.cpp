...
void Result::getEHFieldsModal(Spherical<double> R_, SphericalP<std::complex<double>> &EField_,
  ...
  if(intInd < 0) // Outside a sphere
  {
           ...
           // fields (save it in Einc for convenience)
    {
      ...
      for(size_t j = 0; j < geometry->objects.size(); j++) {
        ...
        Einc =
            Einc + aCoef.dataMp[p_] * geometry->objects[j].sourceCoef[static_cast<int>(p_)] +
    ...
    for(size_t j = 0; j < geometry->objects.size(); j++) {
      ...
      if(singleComponent_ == 1) // TE Part only
      {
        ...
        Hfield =
            Hfield +
            aCoef.dataMp[p_] * scatter_coef[p_.max(nMax) + j * 2 * p_.max(nMax) + p_.compound] * iZ;
      }

      if(singleComponent_ == 2) // TM Part only
      {
        Efield = Efield +
                 aCoef.dataNp[p_] * scatter_coef[p_.max(nMax) + j * 2 * p_.max(nMax) + p_.compound];
      ...
      if(!singleComponent_) {
        ...
        Hfield =
            Hfield +
            (aCoef.dataNp[p_] * scatter_coef[j * 2 * p_.max(nMax) + p_.compound] +
             aCoef.dataMp[p_] * scatter_coef[p_.max(nMax) + j * 2 * p_.max(nMax) + p_.compound]) *
                iZ;
      }
    }
  } else // Inside a sphere
  {
    ...
    if(singleComponent_ == 1) // TE Part Only
    {
      ...
      Hfield = Hfield +
               aCoef.dataMp[p_] *
                   internal_coef[p_.max(nMax) + intInd * 2 * p_.max(nMax) + p_.compound] *
                   iZ_object;
    }
    ...
    if(singleComponent_ == 2) // TM Part Only
    {
      Efield =
          Efield +
          aCoef.dataNp[p_] * internal_coef[p_.max(nMax) + intInd * 2 * p_.max(nMax) + p_.compound];
      Hfield =
          Hfield +
          aCoef.dataNp[p_] * internal_coef[intInd * 2 * p_.max(nMax) + p_.compound] * iZ_object;
    }
    ...
    if(!singleComponent_) // Both TE and TM
    {
      Efield = Efield + (aCoef.dataMp[p_] * internal_coef[intInd * 2 * p_.max(nMax) + p_.compound] +
                         aCoef.dataNp[p_] *
                             internal_coef[p_.max(nMax) + intInd * 2 * p_.max(nMax) + p_.compound]);
      Hfield = Hfield +
               (aCoef.dataNp[p_] * internal_coef[intInd * 2 * p_.max(nMax) + p_.compound] +
                aCoef.dataMp[p_] *
                    internal_coef[p_.max(nMax) + intInd * 2 * p_.max(nMax) + p_.compound]) *
                   iZ_object;
    }
...
void Result::getEHFields(Spherical<double> R_, SphericalP<std::complex<double>> &EField_,
  ...
  if(intInd < 0) // Outside a sphere
  {
           ...
           // fields (save it in Einc for convenience)
    {
      ...
      for(size_t j = 0; j < geometry->objects.size(); j++) {
        ...
        for(p = 0; p < pMax; p++) {
          Einc = Einc + aCoef.dataMp[p] * geometry->objects[j].sourceCoef[static_cast<int>(p)] +
    ...
    for(size_t j = 0; j < geometry->objects.size(); j++) {
      ...
      for(p = 0; p < p.max(nMax); p++) {
        Efield = Efield + aCoef.dataMp[p] * scatter_coef[j * 2 * pMax + p.compound] +
  ...
  } else // Inside a sphere
  {
    ...
    for(p = 0; p < p.max(nMax); p++) {
      Efield = Efield + (aCoef.dataMp[p] * internal_coef[intInd * 2 * pMax + p.compound] +
                         aCoef.dataNp[p] * internal_coef[pMax + intInd * 2 * pMax + p.compound]);
      Hfield = Hfield +
               (aCoef.dataNp[p] * internal_coef[intInd * 2 * pMax + p.compound] +
                aCoef.dataMp[p] * internal_coef[pMax + intInd * 2 * pMax + p.compound]) *
                   iZ_object;
    }
  }
...
void Result::centerScattering() {
  ...
  for(size_t i = 0; i < geometry->objects.size(); i++) {
    Spherical<double> Rrel =
        Tools::toPoint(Spherical<double>(0.0, 0.0, 0.0), geometry->objects[i].vR);
    ...
    Algebra::multiplyVectorMatrix(T_AB, 2 * pMax, 2 * pMax, scatter_aux, scatter_fin, consC1,
                                  consC0);
