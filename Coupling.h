...
public:
  ...
  bool regular;          /**< Specifies if we should be using the regular (J) functions. 1-yes, 0-no. */
   ...
   */
  Coupling(Spherical<t_real> relR_, t_complex waveK_, bool regular_, t_uint nMax_)
  // yep, regular is not regular because optimet
  : nMax(nMax_), relR(relR_), waveK(waveK_), regular(not regular_),
    dataApq(Matrix<t_complex>::Zero(Tools::iteratorMax(nMax), Tools::iteratorMax(nMax))),
    dataBpq(Matrix<t_complex>::Zero(Tools::iteratorMax(nMax), Tools::iteratorMax(nMax))) {}
