 ...
 * The groupID will specify a group in which the sub-groups (r, theta, phi) and (real, imag)
 * will be created.
...
{
private:
  hid_t vecGroupId[3];  /**< The Group IDs for each of the three subgroups. Internal. */
  hid_t vecDataId[6];   /**< The Data IDs for each of the six datasets. Internal. */
  hid_t vecDSpaceId[6]; /**< The Data IDs for each of the six dataspaces. Internal. */
...
public:
  ...
  double *gridParameters; /**< The gridParameters (size and values depend on grid type. */
   ...
   * @param parameters_ the parameters (depending on the grid type). See class docs.
