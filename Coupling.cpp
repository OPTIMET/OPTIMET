...
namespace optimet {
  ...
  namespace {
        ...
        int n_max, Matrix<t_complex> &dataApq, Matrix<t_complex> &dataBpq)
    {
      ...
      auto const m_Matsize1 = 2*(n_max+e)+1;         // up to and including (n_max+e)  : indexed from 0 + 1 for m==0
            ...
            for(int l=1; l<n_Matsize1-e; l++){
              for(int jj=e, k=-n_max; jj<m_Matsize1-e; jj++, k++){
                ...
                if(std::abs(k)<=l){
                  ...
                  dataApq(p, q)
                    = coefficients_A(l, n, j - n_max - e, jj - n_max - e, n_max, AlBe_nmlk);
                  dataBpq(p, q)
                    = coefficients_B(l, n, j - n_max - e, jj - n_max - e, n_max, AlBe_nmlk);
                }
