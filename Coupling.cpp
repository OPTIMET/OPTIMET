...
void Coupling::TransferCoefficients(Spherical<double> R,
                                    std::complex<double> waveK, int BHreg,
                                    int n_max, std::complex<double> **dataApq,
          ...
          for (ii = 1, l = 1; ii < n_Matsize1 - e; ii++, l++) {
            for (jj = e, k = -n_max; jj < m_Matsize1 - e; jj++, k++) {
              ...
              if (std::abs(k) > l) {
                ...
                A2 = sqrt((d_n - d_m) * (d_n + d_m + 1.) * (d_l - d_k) *
                          (d_l + d_k + 1.));
                A3 = sqrt((d_n + d_m) * (d_n - d_m + 1.) * (d_l + d_k) *
                          (d_l - d_k + 1.));
                ...
                // Bnmlk ---------------------------------------------------
                // obtain three coefficients -------------------------------
                B1 = 0.5 * sqrt(((2. * d_l + 1.) / (2. * d_l - 1.)) *
                                (1. / (d_l * (d_l + 1.) * d_n * (d_n + 1.))));
                B2 = sqrt((d_n - d_m) * (d_n + d_m + 1.) * (d_l - d_k) *
                          (d_l - d_k - 1.));
                B3 = sqrt((d_n + d_m) * (d_n - d_m + 1.) * (d_l + d_k) *
                          (d_l + d_k - 1.));
                Bnmlk[i][j][ii][jj] = 2. * d_m *
                                          sqrt((d_l - d_k) * (d_l + d_k)) *
                                          AlBe_nmlk[i][j][ii - 1][jj] +
...
Coupling::Coupling() { initDone = false; }
